  ...
  public class HttpReverseProxyHandler : IHttpHandler //, IHttpAsyncHandler // for later
  {
            ...
            using (HttpWebResponse response = webResponse)
            {
              ...
              if (authenticationChecker != null && authenticationChecker.RootUrl == remoteApplication.RootUrl)
              {
                ...
                if (!string.IsNullOrEmpty(result))
                {
                  ...
                  Authentication.CreateAuthenticationCookie(context.Response, auth.UserId, context.Request.IsSecureConnection, "SecClass3"); // ignore UserData
                }
              }
      ...
      Stream inputBuffer)
    {
      ...
      if (ex.Status == WebExceptionStatus.Timeout)
      {
        ...
        context.Response.End();
        /*
                    408 Request Timeout Der Server hat eine erwartete Anfrage nicht innerhalb des daf�r festgelegten Maximalzeitraums erhalten. Die Verbindung zum anfragenden Browser wird deshalb abgebaut. Angeforderte Daten werden nicht �bertragen. 
                    412 Precondition Failed Eine oder mehrere Bedingungen, die bei der Anfrage gestellt wurden, treffen nicht zu. Die angeforderten Daten werden deshalb nicht �bertragen. 
                    ...
                    502 Bad Gateway Zum Bearbeiten der Anfrage musste der Server einen anderen Server aufrufen, erhielt dabei jedoch eine Fehlermeldung. Die angeforderten Daten k�nnen deshalb nicht gesendet werden. 
                    503 Service Unavailable Der Server kann die Anfrage wegen �berlastung nicht bearbeiten. Die angeforderten Daten k�nnen deshalb nicht gesendet werden. In der Statusmeldung kann stehen, wann die Anfrage fr�hestens wieder bearbeitet werden kann. Im Gegensatz zum Status-Code 202 verarbeitet der Server die Daten nicht, sobald er wieder Kapazit�ten hat. 
                    504 Gateway Timeout Zum Bearbeiten der Anfrage musste der Server einen anderen Server aufrufen, erhielt dabei jedoch nach einem festgelegten Maximalzeitraum keine Antwort. Die angeforderten Daten k�nnen deshalb nicht gesendet werden. 
                    */
      }
      else
      {
